...
class Annonces extends Database
{
  ...
  {
    $dbh = $this->getPdo();
    $sql = 'SELECT utilisateur.nom, utilisateur.prenom, utilisateur.courriel, utilisateur.telephone, annonces.titre, annonces.description, annonces.prix, annonces.id_categorie, annonces.img_url FROM annonces INNER JOIN utilisateur ON annonces.id_utilisateur = utilisateur.id WHERE annonces.uuid = :uuid';
